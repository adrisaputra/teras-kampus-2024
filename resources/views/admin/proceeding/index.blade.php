            ...
            <div class="card">
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-proceeding-table-toolbar="base">
                            ...
                            <a class="btn btn-primary btn-sm me-2 mb-2" data-bs-toggle="modal" data-bs-target="#kt_modal_add_proceeding" onClick="clearForm()"><i class="fa fa-plus"></i>Tambah {{ $title }}</a>
                        </div>
                    </div>
                </div>

                @include('admin.proceeding.create')
											   
                <div class="card-body pt-0">

                    <!--begin::Table-->
                    <table class="table table-striped table-rounded border border-gray-300 table-row-bordered table-row-gray-300 gy-2 gs-6" id="proceeding-table">
...
<script>
    ...
    $(document).ready(function () {
        table = $('#proceeding-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('proceeding.list') }}",
            columns: [
        ...
        $('#myForm').submit(function (e) {
            ...
            var url = "{{ url('/proceeding/validation') }}";
            $.ajax({
                url: url + "/" + action,
                type: "POST",
                data: formData,
    ...
    function send() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#kt_modal_add_proceeding').modal('hide');
    ...
    function getData(id){
        ...
        var url = "{{ url('/proceeding/edit') }}";
        $.ajax({
            url: url + "/" + id,
            type: "GET",
            success: function (response) {
                document.getElementById("id_proceeding").value = response.data.id;
                ...
                if(response.data.cover){
                    var coverLink = '<br><a href="{{ asset("upload/proceeding/") }}/' + response.data.cover + '" class="btn mb-2 mr-1 btn-sm btn-info snackbar-bg-info" target="_blank">Lihat Cover Sebelumnya</a>';
                    document.getElementById("show_cover").innerHTML = coverLink;
                }

                if(response.data.file){
                    var fileLink = '<br><a href="{{ asset("upload/proceeding/") }}/' + response.data.file + '" class="btn mb-2 mr-1 btn-sm btn-info snackbar-bg-info" target="_blank">Lihat File Sebelumnya</a>';
    ...
    function update(id) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#kt_modal_add_proceeding').modal('hide'); // Tutup modal setelah berhasil memperbarui data
