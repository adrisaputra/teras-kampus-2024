...
<form id="myForm" action="{{ url('/'.Request::segment(1)) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
    {{ csrf_field() }}
    <!--begin::Card body-->
    <input type="hidden" class="form-control" id="id_page" value="{{ $page->id }}"/>
    <div class="card-body border-top p-9">
        <!--end::Input group-->
        <div class="row mb-6">
            <label class="col-lg-4 col-form-label fw-bold fs-6">Judul</label>
            <div class="col-lg-8 fv-row">
                <input type="text" name="title" id="title" class="form-control" placeholder="Judul" value="{{ $page->title }}" disabled/>
            </div>
        </div>
        ...
        <div class="row mb-6">
            ...
            <div class="col-lg-8 fv-row">
                <textarea name="text" id="text" class="form-control ckeditor">{{ $page->text }}</textarea>
        ...
        <div class="row mb-6">
            ...
            <div class="col-lg-8 fv-row">
                ...
                @if($page->image)
                    <img src="{{ asset('upload/page/'.$page->image) }}" width="40%">
                @endif
            </div>
        </div>
        @endif
