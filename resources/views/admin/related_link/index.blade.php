            ...
            <div class="card">
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-related_link-table-toolbar="base">
                            ...
                            <a class="btn btn-primary btn-sm me-2 mb-2" data-bs-toggle="modal" data-bs-target="#kt_modal_add_related_link" onClick="clearForm()"><i class="fa fa-plus"></i>Tambah {{ __($title) }}</a>
                        </div>
                    </div>
                </div>

                @include('admin.related_link.create')
											   
                <div class="card-body pt-0">

                    <!--begin::Table-->
                    <table class="table table-striped table-rounded border border-gray-300 table-row-bordered table-row-gray-300 gy-2 gs-6" id="related_link-table">
                        <thead style="background-color: #257180;">
                            <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                                ...
                                <th style="color: white;border-bottom: white;">URL</th>
...
<script>
    ...
    $(document).ready(function () {
        table = $('#related_link-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('related_link.list') }}",
            columns: [
    ...
    function send() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#kt_modal_add_related_link').modal('hide');
    ...
    function getData(id){
        ...
        var url = "{{ url('/related_link/edit') }}";
        $.ajax({
            url: url + "/" + id,
            type: "GET",
            success: function (response) {
                document.getElementById("id_related_link").value = response.data.id;
    ...
    function update(id) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#kt_modal_add_related_link').modal('hide'); // Tutup modal setelah berhasil memperbarui data
        ...
        new Swal({
            ...
            if (result.isConfirmed) {
                new Swal(
                    ...
                    var url = "{{ url('/related_link/delete') }}";
                    $.ajax({
                        url: url + "/" + id,
                        success: function (response) {
